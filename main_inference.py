...
#Interfaz para visión
class IVision(ABC):
    @abstractmethod
    def process(self, image):
        pass

#Sujeto Real para Visión
class MediaPipeHolistic(IVision):
    def __init__(self):
...
#Proxy para Visión
class VisionProxy(IVision):
    def __init__(self):
        self._real = None

    def process(self, image):
        if self._real is None:
            self._real = MediaPipeHolistic()
        return self._real.process(image)
...
vision_proxy = VisionProxy() # Usamos el proxy de visión
sequence = []
sentence = []
predictions = [] 
threshold = 0.7
...
#Proxy ya gestiona el ciclo de vida de la herramienta
while cap.isOpened():
    ret, frame = cap.read()
    if not ret: break

    #Usamos el proxy de visión en lugar de la función suelta
    image, results = vision_proxy.process(frame)
    ...
    if len(sequence) == 30 and frame_skip % 5 == 0:
        ...
        if len(predictions) >= 10:
            ...
            if recent_preds.count(most_common) > 5: #70% de seguridad temporal
